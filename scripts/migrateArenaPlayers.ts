...
async function migrate() {
    ...
    try {
        ...
        // Only process documents where accounts still contains objects (old format)
        const cursor = users.find({ "accounts.0": { $type: "object" } });
        ...
        for await (const doc of cursor) {
            ...
            try {
                ...
                const oldAccounts = doc.accounts as Array<Record<string, unknown>>;
                const newAccounts: number[] = [];
                let primaryAllyCode: number | null = null;
                ...
                for (const entry of awAllyCodes) {
                    ...
                    // Rebuild allyCodes entry without player-data fields
                    const cleanEntry: Record<string, unknown> = { allyCode: entry.allyCode, poOffset: entry.poOffset };
