...
module.exports = (client) => {
    ...
    async function addUser( userId, allyCode ) {
        try {
            ...
            if (!user || !user[0]) {
                ...
                user = await cache.put("swgohbot", "users", {id: userId}, defSettings);
            } else {
                user = user[0];
            }
            return user;
    ...
    async function getUserFromAlly(allyCode) {
        ...
        const users = await cache.get("swgohbot", "users", {"accounts.allyCode": allyCode});
    ...
    async function updateUser(userId, userObj) {
        ...
        let newUser = await cache.put("swgohbot", "users", {id: userId}, userObj);
    ...
    async function removeAllyCode(userId, allyCode) {
        ...
        return await cache.put("swgohbot", "users", {id: userId}, user);
    }

    async function removeUser(userId) {
        // Completely wipe a user? 
        const res = await client.mongo.db("swgohbot").collection("users").deleteOne({id: userId})
            .then(() => { return true; })
            .catch(() => { return false; });
        return res;
    }
