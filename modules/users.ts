...
class UserReg {
    ...
    async getUser(userId: string) {
        const user = await this.cache.getOne<UserConfig>(config.mongodb.swgohbotdb, "users", { id: userId });
        return user || null;
    }

    async getUsersFromAlly(allyCode: string | number) {
        const allyCodeStr = Number(allyCode).toString();
        const users = await this.cache.get<UserConfig>(config.mongodb.swgohbotdb, "users", { "accounts.allyCode": allyCodeStr });
        return users?.length ? users : null;
    }

    async updateUser(userId: string, userObj: UserConfig) {
        const newUser = await this.cache.put<UserConfig>(config.mongodb.swgohbotdb, "users", { id: userId }, userObj);
        return newUser;
    }
