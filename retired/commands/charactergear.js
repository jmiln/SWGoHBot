...
class Charactergear extends Command {
    ...
    async run(Bot, message, [userID, ...searchChar], options) {
        ...
        if (!userID) {
            ...
            const allycode = await Bot.getAllyCode(message, userID);
            userID = allycode;
            if (!allycode) {
        ...
        if (!userID) {
            if (!gearLvl) {
                ...
                const msgArr = Bot.msgArray(message.language.get("COMMAND_CHARACTERGEAR_GEAR_ALL", character.name, gearString), "\n", 1900);
            ...
            let player = await Bot.swgohAPI.unitStats(userID, cooldown);
