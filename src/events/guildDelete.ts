...
module.exports = async (Bot: BotType, guild: Guild) => {
    ...
    await Bot.database.models.settings.destroy({where: {guildID: guild.id}})
        .then(() => {})
        .catch((error: Error) => { Bot.logger.error(`ERROR: Broke in guildDelete(settings) ${error}`); });
    ...
    await Bot.database.models.eventDBs.destroy({where: {eventID: { [Bot.seqOps.like]: `${guild.id}-%`}}})
        .then(() => {})
        .catch((error: Error) => { Bot.logger.error(`ERROR: Broke in guildDelete(eventDBs) ${error}`); });
