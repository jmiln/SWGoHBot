...
module.exports = async (Bot: BotType, client: BotClient) => {
    ...
    if (client.shard) {
        readyString = `${client.user.username} is ready to serve ${client.users.cache.size} users in ${client.guilds.cache.size} servers. Shard #${client.shard.ids[0]}`;

        // Connect the sockets and such
        Bot.socket = io(`ws://localhost:${Bot.config.eventServe.port}`);
        Bot.socket.on("connect", () => {
            console.log(`[EventMgr](${client.shard.ids[0]}) Connected to socket!`);
        });
        Bot.socket.on("disconnect", () => {
            console.log(`[EventMgr](${client.shard.ids[0]}) Disconnected from socket!`);
        });
